...
public function affectiveDomain(Request $request)
{
    $schoolId = $request->header('X-School-ID');
    ...
    return response()->json( $affective, 200);
}


public function getAffectiveClassScores(Request $request)
{
    $schoolId = $request->header('X-School-ID');
    ...
    $scores = AffectiveScore::with('type')
...
public function saveDomain(Request $request)
{
    ...
    $validated = $request->validate([
        ...
        'domainType' => 'required|string|in:Affective,Psychomotor',
    ...
    {
        $domain = AffectiveDomain::findOrFail($id);
        $domain->delete();
...
public function storeDomainScores(Request $request, int $domainId): JsonResponse
{
    ...
    $domain = AffectiveDomain::findOrFail($domainId); 
    // If separate tables: AffectiveDomain::findOrFail($domainId)
    ...
    {
        $scores = AffectiveScore::with('domain', 'subject')
